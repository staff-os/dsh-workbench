...
let WorkbenchSkillGateway = (() => {
	...
	return class WorkbenchSkillGateway extends _classSuper {
		static {
			...
			_importUrl_decorators = [Remote("importUrl")];
			...
			__esDecorate(this, null, _importUrl_decorators, {
				kind: "method",
				name: "importUrl",
				static: false,
				private: false,
				access: {
					has: (obj) => "importUrl" in obj,
					get: (obj) => obj.importUrl
				},
				metadata: _metadata
			}, null, _instanceExtraInitializers);
		...
		* @returns 装好的技能与刷新后的快照。
		*/
		async importUrl(url, overwrite, name) {
			const origin = classifyImportSource(url.trim());
			if (origin.kind !== "url") throw new WorkbenchError("只收 http(s) 链接：市场条目请用安装，本地压缩包请用上传", "WORKBENCH_BAD_ARG");
			const runtime = this.runtime();
			const picked = selectSkillFromPackage(await fetchPackage(origin.url, origin.label, runtime.registryTimeoutMs), name);
			const result = await installSkillFiles({
				root: this.root(),
				stagingParent: runtime.paths.skillStaging
			}, picked.files, { overwrite: overwrite === true });
			const resources = result.fileCount - 1;
			return this.mutated(projectLocal(result.skill, false), `已${result.replaced ? "覆盖安装" : "安装"}技能 "${result.installedAs}"（来自 ${origin.label}${resources > 0 ? `，含 ${String(resources)} 个资源文件` : ""}${result.binaryCount > 0 ? `，其中 ${String(result.binaryCount)} 个二进制` : ""}）`, result.installedAs);
		}
		/**
		* 查哪些已装技能有新版本。
		*
